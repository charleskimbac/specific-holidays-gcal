/**
 * Returns a subset of the given iCal (ICAL_LINK) of only the 
 * holiday events provided by the user (SPECIFIC_HOLIDAYS).
 *
 * @return Parsed iCal containing events of the specified holidays.
 */
function getSpecificHolidaysICal() {
  const response = UrlFetchApp.fetch(ICAL_LINK);
  const data = response.getContentText();

  const jCalAll = ICAL.parse(data);
  const allEvents = jCalAll[2];

  const specificEvents = [];
  allEvents.forEach(event => {
    if (SPECIFIC_HOLIDAYS.includes(event[1][10][3])) {
      specificEvents.push(event);
    }
  });

  const iCalSpecific = [];
  iCalSpecific[0] = jCalAll[0];
  iCalSpecific[1] = jCalAll[1];
  iCalSpecific[2] = specificEvents;
  const iCal = ICAL.stringify(iCalSpecific);

  // format for use
  const iCalSpecificParsed = [[]];
  iCalSpecificParsed[0][0] = iCal;
  return iCalSpecificParsed;
}
